...
(deftest join-parser
  (testing "Join deve retornar dados na estrutura correta"
    (is (= (-> (get-flat-tree (parse-inner-join-conditions ["VAL_1" "=" "VAL_2"])) :condicoes first :campo) "VAL_1"))
    (is (= (-> (get-flat-tree (parse-inner-join-conditions ["VAL_1" "=" "VAL_2"])) :condicoes first :operador) :eq))
    (is (= (-> (get-flat-tree (parse-join [["INNER" "JOIN" "TABELA_TESTE" "ON" "VAL_1" "=" "VAL_2"] '{}])) :join :tabela) "TABELA_TESTE"))
    (is (= (count (get-tokens (parse-join (parse-data-source (parse-columns (parse-operation (tokenize operacao-com-join))))))) 0))
    (is (= (-> (get-flat-tree (parse-join (parse-data-source (parse-columns (parse-operation (tokenize operacao-com-join)))))) :join :tipo) :inner))
    (is (= (-> (get-flat-tree (parse-join (parse-data-source (parse-columns (parse-operation (tokenize operacao-com-join)))))) :join  :tabela) "OTHER_THING"))))
